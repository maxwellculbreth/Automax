      ...
      <div className="mx-auto max-w-3xl bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
          ...
          <div>
            ...
            {quote.expires_at ? (
              ...
              >
                ...
                {expiryWarning === 'expired' && (
                  <span className="ml-2 text-[11px] font-semibold text-red-500">(Expired)</span>
                )}
                {expiryWarning === 'near' && (
                  <span className="ml-2 text-[11px] font-semibold text-amber-500">(Expiring soon)</span>
                )}
              </div>
            ) : (
        ...
        <div className="px-8 py-6 border-b border-border grid sm:grid-cols-2 gap-6">
          <div>
            ...
            {quote.customer_email && (
              <div className="text-[13px] text-muted-foreground">{quote.customer_email}</div>
            )}
            {quote.customer_phone && (
              <div className="text-[13px] text-muted-foreground">{quote.customer_phone}</div>
            )}
          </div>
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">
        ...
        <div className="px-8 py-6 border-b border-border">
          ...
          <div className="hidden sm:grid sm:grid-cols-[1fr_60px_100px_100px] gap-4 mb-2 pb-2 border-b border-border/60">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Item</div>
            <div className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground text-right">Qty</div>
            <div className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground text-right">Unit Price</div>
            <div className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground text-right">Total</div>
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-border space-y-2 sm:max-w-xs sm:ml-auto">
            ...
            {discountAmt > 0 && (
              <div className="flex items-center justify-between">
                ...
                <span className="text-[13px] text-emerald-600 tabular-nums font-medium">
            ...
            {quote.deposit_required && quote.deposit_amount > 0 && (
              <div className="flex items-center justify-between pt-1">
                <span className="text-[12px] text-muted-foreground">
                  Deposit required
                  {quote.deposit_type === 'percent' && ` (${quote.deposit_amount}%)`}
                </span>
