...
// Exported so the page can reference the same column metadata for the insights panel
...
/** Sort visible cards: most recent contact → highest value → most recently created */
      ...
      {KANBAN_COLUMNS.map(column => {
        ...
        return (
          <div
            key={column.id}
            className={cn(
              // Fixed width + fixed height when collapsed so all lanes are identical
              "w-[248px] flex-shrink-0 flex flex-col",
              "rounded-xl border border-border/60 bg-card shadow-sm border-t-2",
              column.accentBorder,
              isExpanded ? "min-h-[420px]" : "h-[420px]",
          ...
          >
            {/* Column header — fixed, does not grow */}
            <div className="flex-shrink-0 px-3.5 pt-3 pb-2.5 border-b border-border/40">
              ...
              <p className={cn(
                "mt-0.5 text-[12px] font-medium tabular-nums pl-4 transition-opacity",
                columnValue > 0 ? "text-muted-foreground opacity-100" : "opacity-0 select-none"
              )}>
                {columnValue > 0 ? formatCurrency(columnValue) : "$0"}
              </p>
            </div>

            {/*
              Card area — flex-1 so it fills remaining column height.
              Cards sit at top; empty space below acts as a visual drop zone.
              When expanded, becomes scrollable and the column grows.
            */}
            <div className={cn(
            ...
            {hiddenCount > 0 && (
              <div className="flex-shrink-0 border-t border-border/40 px-3 py-2">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 rounded-md py-1 text-[11px] font-medium text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-colors"
                >
                  {isExpanded
...
// Empty lane fills the full card area (flex-1) so the column height stays consistent
function EmptyLane({ isDragTarget, onAddLead }: { isDragTarget: boolean; onAddLead: () => void }) {
  return (
    <div className={cn(
      "flex flex-1 flex-col items-center justify-center gap-3 m-2.5 rounded-lg border-2 border-dashed transition-colors",
      isDragTarget ? "border-primary/40 bg-primary/5" : "border-border/40"
    )}>
      <p className="text-[11px] font-medium text-muted-foreground/40">No leads here</p>
      <button
        onClick={onAddLead}
        className={cn(
          "flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[11px] font-medium",
          "text-muted-foreground border border-border/60 transition-all",
          "hover:text-foreground hover:border-border hover:bg-secondary/50 hover:shadow-sm"
        )}
      >
    ...
    >
      <div className="flex items-start gap-1.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          {lead.service && (
            <p className="text-[11px] text-muted-foreground mt-0.5 truncate">{lead.service}</p>
          )}
          <div className="flex items-center justify-between mt-2.5 gap-2">
            <span className={cn(
              "text-[13px] font-bold tabular-nums",
              lead.estimated_value ? "text-foreground" : "text-muted-foreground/35"
            )}>
